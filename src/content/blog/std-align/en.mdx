...
tags:
  - code
  - cpp
description: "This blog post explains C++ standard library's std::align utility, using an arena allocator as a motivating example"
---

import { Image } from "astro:assets";
import arena1 from "./arena_1.svg";
import arena2 from "./arena_2.svg";
import arena3 from "./arena_3.svg";
import arena4 from "./arena_4.svg";
...
With arena, we start with a large chunk of pre-allocated memory coming from either the stack or another allocator such as `mmap` or `malloc`.
Afterward, we allocate memory from that chunk by bumping a pointer offset.

<aside class="side-note" style={{marginTop: "-130px"}}>
...
<figure>
  <Image src={arena1} width="500" alt="Arena before allocation" class="block m-20-auto" />
  <figcaption style="text-align: center">Figure.1 - Arena before allocation</figcaption>
</figure>

<figure>
  <Image src={arena2} width="500" alt="Arena after allocation" class="block m-20-auto" />
...
Each allocation only requires a pointer bump, and the deallocation is almost free as long as the objects allocated are _trivially destructible_.
...
<aside class="side-note" style={{marginTop: "-110px"}}>

In C++, a type is _trivially destructible_ if it doesn't have a destructor that performs actions. For example, `std::string` and `std::vector` are not trivially destructible since their destructors free memory. Everything that contains non-trivially destructible types are also not trivially destructible.
...
The simple solution above would be perfect if _alignment_ doesn't exist.
...
<aside class="side-note" style={{marginTop: "-65px"}}>

See how easily undefined behavior, one of the most dreaded things among our C++ programmers, can creep in when we mess with raw memory? There is a reason why we want to encapsulate memory allocations.
...
<aside class="side-note" style={{ marginTop: "-110px" }}>
  <a href="https://en.cppreference.com/w/cpp/numeric/bit_cast">
    <code>std::bit_cast</code>
  </a>{" "}
  is a C++20 feature. Before C++20, you need <code>reinterpret_cast</code>.
</aside>
...
<figure>
  <Image
    src={arena4}
    alt="Alignment-aware arena after allocating one uint8_t and one uint32_t"
    class="block m-20-auto"
  />
