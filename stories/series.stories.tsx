...
const Series = (props: { seriesTitle: string, entries: SeriesEntry[] }) => {
  ...
  return <Container>
    <Title>This post is a part of {seriesTitle} ({entries.length} parts series)</Title>
    {
      entries.map((e, i) => {
        const title = `${e.title}`;
        return <Item to={e.slug}>
          {i+1}) { title }
        </Item>;
      }
      )
...
const eight: SeriesEntry[] =
  [
    { title: "Judgements and Inference Rules", slug: "/judgements-inference-rules" },
...
export const Eight = () => <Series seriesTitle="Fundamental of Programming Languages" entries={eight} />;
